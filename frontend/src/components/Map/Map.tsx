...
const MapInstance = () => {
  ...
  const [routeInstructions, setRouteInstructions] =
    useState<RouteInstruction[]>();
  ...
  useEffect(() => {
    if (hoverInfo && state.routeInstructions) {
      if (hoverInfo.layerId.includes("waypoint")) {
        ...
        const ins = findWaypointByCoords(
          hoverInfo.layerId,
          coords,
        ...
        if (ins) {
          dispatch({
            ...
            selectedWaypoint: { coords, instruction: ins?.instruction },
          });
        } else {
          dispatch({
            type: "SET_SELECTED_WAYPOINT",
  ...
  useEffect(() => {
    if (
      ...
      const routeInstructions = state.routeInstructions?.find(
        (instruction) => instruction.id === state.selectedRoute?.id
      );
  ...
  useEffect(() => {
    if (state.route && state.route.length >= 1 && !state.loading) {
      dispatch({
        ...
        selectedRouteId: state.route[0].id,
      });
      ...
      >
        {state.selectedWaypoint && (
          <Marker
            ...
            offset={[0, -60]}
            draggable={false}
            longitude={state.selectedWaypoint.coords[0]}
          ...
          routes
            ...
            .map((route, i) => (
              <RouteSource
                ...
                id={route.id}
                waypoints={
                  routeInstructions.slice().reverse()[i].waypointCoords
                }
              />
            ))}
