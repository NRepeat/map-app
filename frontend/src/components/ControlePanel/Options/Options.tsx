...
const Options = () => {
  ...
  const [selectedAvoidKeys, setSelectedAvoidKeys] = useState<Set<any>>(
    selectedRouteOptions
      ? new Set(selectedRouteOptions.avoid_features)
      : new Set([])
  );
  const [selectedPreferenceKeys, setSelectedPreferenceKeys] = useState<
    Set<any>
  >(
    selectedRouteOptions && selectedRouteOptions.preference
      ? new Set([selectedRouteOptions.preference])
      : new Set(["recommended"])
  );
  const [maximumSpeed, setMaximumSpeed] = useState<string>(
    selectedRouteOptions && selectedRouteOptions.maximum_speed
      ? selectedRouteOptions.maximum_speed.toString()
      : ""
  );
  const [selectedUnit, setSelectedUnit] = useState<UnitsType>(
    selectedRouteOptions ? selectedRouteOptions.units! : "m"
  );
  const [continueStraight, setContinueStraight] = useState<boolean>(
    selectedRouteOptions ? selectedRouteOptions.continue_straight! : false
  );
  const units = ["m", "km", "mi"];

  const setCOptions = useCallback(() => {
    const options: RouteOptions = {
      avoid_features: Array.from(selectedAvoidKeys),
      continue_straight: continueStraight,
      preference: Array.from(selectedPreferenceKeys)[0],
      units: selectedUnit,
    };
    if (maximumSpeed) {
  ...
  return (
    <Card>
      <Button
        ...
        color={toggleMenu ? "success" : "default"}
      >
        Options
      </Button>
      <AnimatePresence>
