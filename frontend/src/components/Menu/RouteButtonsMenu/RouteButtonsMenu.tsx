      ...
      <ButtonGroup fullWidth>
        ...
        >
          <Button
            ...
            radius="md"
            disabled={
              (state.route && state.route.length >= 1) || state.selectedRoute
                ? false
                : true
            }
          ...
          >
            Get route{" "}
            <MdRoute
              className={`w-5 h-5 ${state.loading ? "animate-pulse " : ""}`}
            />
          </Button>
        </Tooltip>
        <Button
          color={
            state.markers && state.markers.length >= 2 ? "secondary" : "default"
          }
          disabled={state.route && state.route.length >= 1 ? false : true}
          onClick={() => handleGetOptimizedRoute()}
        >
          Get optimization{" "}
          <BsGearWide className={`${state.loading ? "animate-spin " : ""}`} />
        </Button>
        {(state.route && state.route?.length >= 1) ||
          (state.selectedRoute && (
            <Button
              onClick={() => handleClearInputsState()}
              variant="solid"
              color="danger"
            >
              Clear route
