...
const CordList = () => {
	...
	const startIcon = (
		<FaMapMarkerAlt className="fill-green-700 sm:min-w-3 sm:min-h-3  min-h-2 min-w-2 " />
	);
	const endIcon = (
		<FaMapMarkerAlt className="fill-red-600 sm:min-w-3 sm:min-h-3  min-h-3 min-w-3 " />
	);
			...
			{state.places &&
				state.places.map((data, i) => {
					if (state.places) {
						const isStart = i === 0;
						const isEnd =
							state.places.length >= 2 && i === state.places.length - 1;
						const startContent = isStart
							? startIcon
							: isEnd
								? endIcon
								: i !== 0 && i !== state.places.length - 1 && i;
						return (
							<div key={data.id} className="inline-flex gap-2">
