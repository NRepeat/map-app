...
const RouteCard = ({ route, isLoaded, showing }: { route: RouteType, isLoaded?: boolean, showing?: boolean }) => {
	...
	return (
		<div onClick={handelOpenRouteInstructions} onMouseOver={handleSelectRoute} className='cursor-pointer'>
			<Card className={` border-2 transition-border duration-500 ${isSelected ? 'border-emerald-500' : 'border-gray-300 border-opacity-0'} hover:border-emerald-500 hover:border-2`}>
				<CardHeader className='justify-between items-center'>
					...
					<div className='flex items-center min-h-[64px] flex-col-reverse justify-center '>
						{isLoaded ? <>
							{selectedRouteId && selectedRouteId === route.id && !showing ? <Button onClick={handleLoad}> Show route</Button> : <> 	{selectedRouteId && selectedRouteId === route.id &&
					...
					<div className='flex pt-2 pl-0.5  text-lg  justify-between w-full text-balance'>
						<div className='flex flex-col w-full  font-light'>
							...
							<span>
								...
								{options?.maximum_speed && <p className='text-pretty'>
